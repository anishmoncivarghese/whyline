...
- **CARRY INTO THE NEXT RELEASE, whichever it is: PyPI's 0.1.3 page still says the read-side rate is 67%.** The real figure is 50% and git is correct; a README is baked into the published artifact, so 0.1.3 cannot be amended and PyPI forbids re-uploading it. Decided 2026-08-19 to let the correction ride along with the next substantive release rather than cut a version whose only content is prose. **This is a published overstatement of a measured result — do not let it ship again uncorrected.** No action needed beyond releasing; the corrected README is already on `main`.
