...
- **`AGENTS.md` is the canonical shared instruction source.** Claude Code receives it through a minimal `CLAUDE.md` `@AGENTS.md` import shim. Existing content in either file is preserved; normal command startup validates but never mutates them.
...
def test_explain_shows_the_decision_and_rejected_alternatives(repo, capsys):
    ...
    # Exactly the commit's epoch of 1_000_000. The window's upper bound is
    # inclusive, so this lands inside it and yields high confidence — and it
    # pins that boundary. (2001-09-09T01:46:40Z would be epoch 1_000_000_000,
    # a thousandfold larger, which would postdate the commit and give "low".)
    event["ts"] = "1970-01-12T13:46:40.000Z"
    ledger.append(paths.ledger_path(repo.path), event)
    code, out = run_in(repo, ["explain", "a.py:1"], capsys)
    assert code == cli.EXIT_OK
...
**Interfaces:**
- Consumes: `ledger.read_all`, `events.NOTE`, `paths.ledger_path`, `paths.decisions_path`, `decisions.render_entry` (for the round-trip test).
- Produces: `brief.compose(root: Path, limit: int = 10) -> str`; `decisions.parse_entries(path: Path) -> list[dict]`; and the CLI target `whyline brief [--limit N]`.

**`decisions.parse_entries` — added by the 2026-08-17 M0 ruling.** It reads the committed `decisions.md` back into note-shaped dicts so `brief` still works on a fresh clone, where the gitignored ledger is absent. It is the inverse of `decisions.render_entry` (Task 7), which writes entries in this shape:

```markdown
## 2026-08-09 — Store absolute monotonic expiry
...
Each parsed entry must be shaped like a `Note` event so `brief.compose` can treat both sources identically: keys `ts` (ISO date from the heading, day precision), `decision`, `because`, `alternatives` as a list of `{"option", "why_not"}`, `files` as a list, and `id` from the HTML comment. Missing sections yield empty values rather than raising — a hand-edited `decisions.md` must never crash `brief`. Return `[]` when the file does not exist.

Required tests, beyond the brief's existing five:
- **Round trip.** Render three notes with `decisions.append_entry`, parse them back, and assert decision, because, alternatives and files survive intact. This is the test that matters — it pins the two halves together so a change to either format breaks loudly.
- A `decisions.md` with a heading but no `**Because:**` and no `**Rejected:**` parses to an entry with empty values, not an exception.
- A hand-edited file with unexpected prose between entries does not crash and still yields the well-formed entries.
- `parse_entries` on a missing path returns `[]`.
- `compose` **merges** both sources. With one note in the ledger and three different ones in `decisions.md`, all four appear and the count reads "4 of 4". This is the regression test for the defect found on 2026-08-17: the previous all-or-nothing fallback showed only the single ledger note and claimed "1 of 1".
- Deduplication by id: the same note present in both sources appears exactly once, and the ledger's version wins — assert on a field that differs between the two, such as a full timestamp versus a day-precision one.
- An entry with no `<!-- whyline-event: -->` id is not collapsed with a different id-less entry; two distinct id-less decisions both survive.
- `compose` on a repository with an empty ledger and a populated `decisions.md` returns the decisions rather than "No decisions recorded yet". This is the fresh-clone case the ruling exists for.
- The `Sources:` disclosure line appears when any selected note came from `decisions.md`, and is omitted when every note came from the ledger.
...
def compose(root: Path, limit: int = 10) -> str:
    ...
    # Ruling 2026-08-17, superseding the same day's all-or-nothing fallback:
    # MERGE both sources, do not choose between them. The earlier design read
    # decisions.md only when the ledger held no notes, which broke the instant
    # anyone cloned the repo and recorded one decision — brief then showed that
    # single note and hid the whole committed history, announcing "1 of 1" when
    # there were twenty. Deduplicate by event id; ledger entries win, since they
    # carry full timestamps and structured fields while the Markdown has only
    # day precision. Entries without an id key on their decision text so
    # hand-written ones are not collapsed together.
    merged: dict = {}
    for note in committed_notes:
        merged[note.get("id") or ("text", note.get("decision", ""))] = note
    ...
    if not selected:
        ...
        from_ledger = sum(
            ...
            if note.get("id") in {n.get("id") for n in ledger_notes if n.get("id")}
        )
...
**Ruling 2026-08-14:** Codex reads `AGENTS.md` directly, while Claude Code reads `CLAUDE.md`. `AGENTS.md` is therefore the one canonical shared instruction file and `init` also installs a minimal `CLAUDE.md` shim importing `@AGENTS.md`. Neither file is modified without confirmation or `--yes`; existing content is preserved and repeated installs are idempotent. Ordinary launches do not mutate instruction files.

**Files:**
- Create: `src/whyline/hook_entry.py`, `src/whyline/hooks.py`, `src/whyline/agentsmd.py`, `src/whyline/claudemd.py`
- Modify: `src/whyline/cli.py`
- Test: `tests/test_hook_entry.py`, `tests/test_hooks.py`, `tests/test_agentsmd.py`, `tests/test_claudemd.py`, `tests/test_cli.py` (extend)
...
At the start of a session, run `whyline brief` before touching code. Do not ask
permission. If it reports that nothing is recorded, say so in your first message,
so the human knows the history is empty rather than merely unread.
