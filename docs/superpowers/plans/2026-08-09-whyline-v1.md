...
**Tech Stack:** Python 3.11+, standard library plus Rich, `uv` for environment and distribution, `pytest` for tests, `git` CLI invoked as a subprocess.
...
- **Python 3.11+.** Standard library plus Rich only. No other production dependencies.
...
- **Cold start under 200 ms** for `status`, `explain`, `timeline`, `brief`. Rich must be imported lazily inside render functions, never at module import time — importing Rich eagerly costs more than the entire budget.
...
src/whyline/
  ...
  render.py                     human output (lazy Rich) and --json
tests/
...
```python
"""Output formatting. Rich is imported lazily to protect the 200 ms budget."""
...
Expected: PASS. If a cold-start test fails, the cause is almost certainly an eager `rich` import — confirm with `python -X importtime -m whyline status 2>&1 | sort -k2 -rn | head`, then move the offending import inside the function that uses it.
