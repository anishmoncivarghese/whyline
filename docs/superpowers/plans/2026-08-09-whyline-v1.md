...
- `compose` prefers the ledger when it has notes, and uses `decisions.md` only when the ledger has none — assert with both present and deliberately different content, so preference order is unambiguous.
