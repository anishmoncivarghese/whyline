...
def explain(root: Path, rel_path: str, line: int | None) -> Explanation:
    ...
    if notes:
        # Notes exist for this path but none could be placed in time, so they
        # cannot be tied to this line. Saying "no reasoning recorded" here
        # would be false; report what we actually know.
        return Explanation(
            path=rel_path,
            line=line,
            confidence=LOW,
